...
const WS_URL = 'wss://web-production-7cb21.up.railway.app/ws'
...
export function useBackend() {
  ...
  const fetchHistory = useCallback(async () => {
    try {
      ...
      setReadings(Array.isArray(r) ? r.reverse() : [])   // fix: guard against non-array
      setAlerts(Array.isArray(a) ? a : [])
      setStats(st && typeof st === 'object' ? st : null)
      setStatus(sys && typeof sys === 'object' ? sys : null)
 
      const safeR = Array.isArray(r) ? r : []
      if (safeR.length > 0) setLastReading(safeR[safeR.length - 1])
    } catch (e) {
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      console.warn('WebSocket construction failed:', e.message)
  ...
  const simulate = useCallback(async (scenario = 'normal') => {
    try {
